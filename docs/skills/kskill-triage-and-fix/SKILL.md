...
Upgrades over the early Claude `kskill-kskill-wf-triage-and-fix` cast:
...
4. **Assertions + guardians**: doctrine-first plan, inquisitor TDD gate on
   assertion-touching plans, post-bard `khook-guardian-dispatch` + `kskill-assertion-review`.
...
1. **Guardian dispatch.** From repo root:
   `python3 docs/hooks/khook-guardian-dispatch --bundle <baseRef>`. Paste the
...
2. **Assertion review.** If the combined diff touches `docs/assertions/**`
   ...
   `docs/skills/kskill-assertion-review/SKILL.md`. Unmet → batch not closed; TDD
...
```
docs/skills/kskill-triage-and-fix/bin/kwf-deps requires <pr> <N...> [--repo R] [--dry-run]
docs/skills/kskill-triage-and-fix/bin/kwf-deps check <pr> [--repo R]
docs/skills/kskill-triage-and-fix/bin/kwf-deps cascade <pr> [--repo R] [--dry-run] [--force]
docs/skills/kskill-triage-and-fix/bin/kwf-deps lift <pr> [--repo R] [--dry-run]
docs/skills/kskill-triage-and-fix/bin/kwf-deps status <pr> [--repo R]
```
