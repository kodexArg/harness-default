...
> When kodex invokes `/k-report` with **no markdown attached**, the content
...
7. **Save a local copy & report (MANDATORY — always).** ALWAYS persist the
   ...
   no asking. Canonical archive dir is `~/Documents/k-report/` (create it with
   `mkdir -p` if missing); write `~/Documents/k-report/<slug>-YYYYMMDD.html`. If the
   user named a different target folder in the request, ALSO write a copy there,
   but the `~/Documents/k-report/` archive copy is always written regardless. Tell the
   user the absolute path(s). If a chrome-devtools MCP is available and they want
   a preview, offer to open it. **Never** deliver a report without first having
   written its local copy — the saved file is the artifact.
8. **Close on the path (no delivery channel).** The saved file IS the delivery.
   Report the absolute path and the `<h1>` + one-line summary in chat, and stop
   there. This harness ships **no** send step: no Telegram, no bot token, no
   outbound push. If the caller wants the report somewhere else, they name the
   folder (step 7 writes the extra copy) or move it themselves.
...
```bash
python3 skills/k-report/scripts/validate_mermaid.py <artifact.html>
```
...
- Local copy ALWAYS written to `~/Documents/k-report/` (+ any user-named folder) and its absolute path reported — no report is ever send-only.
- Local path reported in chat (step 8). No send step exists in this harness.
