...
Usage:
    python3 skills/k-live-doc/link.py            # apply + write CODEMAP
    python3 skills/k-live-doc/link.py --check    # report only, non-zero if drift
"""
...
END = "LIVE-DOC:END"
ADR_REF = "[[HARNESS]]"
SLUG = os.environ.get("PROJECT_SLUG", "harness-default")
...
def write_codemap(index: dict[str, list[str]]):
    lines = [
        ...
        "> Regenerated by `skills/k-live-doc/link.py` from each code file's live-doc block. "
        "Do not hand-edit; edit the block (or the manifest) and re-run. Ruled by [[HARNESS]].",
