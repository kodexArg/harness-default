...
`skills/k-triage-and-fix` → `~/.kimi-code/skills/triage-and-fix`).
...
`agents/kwf-<name>.md`, then `Task` with a mapped worker and a prompt that
...
|---|---|
| cheap / low | `kbot-low` (read-only scouts/familiars/priest/shadow) |
| mid | `kbot-medium` |
| high | `kbot-medium` or `generalPurpose` (builders that write) |
| heavy | `kbot-high` or `generalPurpose` (mage, inquisitor, elf-mage, paladin) |

Builders that must edit: use a write-capable worker (`kbot-medium` /
`generalPurpose`), never `kbot-low`. Priest and shadow stay tool-free in
...
Every `kwf-*` / guardian agent may declare `soul: agents/souls/<name>.md`
([[adr-02-harness-layout]] rule 8). On dispatch, **prepend that file** to the node
...
4. Post-bard: `guardian-dispatch --bundle` (paste payload; cheap tier;
   parallel when both owed) + `k-assertion-review` when assertions moved.
5. `bin/kwf-deps` path: `skills/k-triage-and-fix/bin/kwf-deps` from repo root.
