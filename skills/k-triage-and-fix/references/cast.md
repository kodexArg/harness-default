...
- Cast SSOT: `agents/kwf-*.md`. Frontmatter declares `name`, `description`,
...
- **Post-bard hooks** — **in force**: after bard, the orchestrator runs
  `hooks/khook-guardian-dispatch` and `k-assertion-review` when assertions moved. See
  Post-bard in `SKILL.md` and [[adr-04-guardians-and-delivery]]. Verifier / smoke-test pause
  remain open.
