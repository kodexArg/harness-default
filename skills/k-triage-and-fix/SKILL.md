...
description: >
  ...
  the kwf-* cast under agents/. PR REQUIREMENT system (requires:N, defer
...
compatibility: >
  ...
  agents/kwf-*.md. Dispatch mechanics differ by host — see
...
Ships **inside harness-default** (`agents/kwf-*` + this skill). Binding ADR:
`adrs/adr-04-guardians-and-delivery.md` (law: `adr-01`, tooling: `adr-02`).
Runtime spawn/model map:
`references/runtimes.md`.

Upgrades over the early Claude `k-wf-triage-and-fix` cast:
...
4. **Assertions + guardians**: doctrine-first plan, inquisitor TDD gate on
   assertion-touching plans, post-bard `khook-guardian-dispatch` + `k-assertion-review`.
...
1. **Guardian dispatch.** From repo root:
   `python3 hooks/khook-guardian-dispatch --bundle <baseRef>`. Paste the
...
2. **Assertion review.** If the combined diff touches `docs/assertions/**`
   ...
   `skills/k-assertion-review/SKILL.md`. Unmet → batch not closed; TDD
...
```
skills/k-triage-and-fix/bin/kwf-deps requires <pr> <N...> [--repo R] [--dry-run]
skills/k-triage-and-fix/bin/kwf-deps check <pr> [--repo R]
skills/k-triage-and-fix/bin/kwf-deps cascade <pr> [--repo R] [--dry-run] [--force]
skills/k-triage-and-fix/bin/kwf-deps lift <pr> [--repo R] [--dry-run]
skills/k-triage-and-fix/bin/kwf-deps status <pr> [--repo R]
```
...
- A grant is a claim; a prompt is a wish — enforce via `tools:` in `agents/kwf-*.md`.
...
| File | What it is |
|---|---|
| `agents/kwf-*.md` | 18 cast definitions — `tools:` is enforcement |
